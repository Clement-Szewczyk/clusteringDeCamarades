...
def readPreferences(csvFile):
    ...
    finalAffinity = mutualAffinity * 1.5 + unilateralAffinity * 1.0
    ...
    for group in groups:
        ...
        for i in range(len(group)):
            for j in range(i + 1, len(group)):
                ...
                possibleAffinity += 2 * TOTAL_POINTS * 1.5  # Max with mutual bonus
...
def main():
    ...
    try:
        ...
        print(f"    Each student distributes {TOTAL_POINTS} points")
        ...
        print("  Error: Could not find 'backend/algo/preferences_weighted.csv'")
